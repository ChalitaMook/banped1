@extends('layouts.bootstrap')


<style>
    body {
        margin-top: 20px;
        color: #1a202c;
        text-align: left;
        background-color: #e2e8f0;
    }

    .main-body {
        padding: 15px;
    }

    .card {
        box-shadow: 0 1px 3px 0 rgba(0, 0, 0, .1), 0 1px 2px 0 rgba(0, 0, 0, .06);
    }

    .card {
        position: relative;
        display: flex;
        flex-direction: column;
        min-width: 0;
        word-wrap: break-word;
        background-color: #fff;
        background-clip: border-box;
        border: 0 solid rgba(0, 0, 0, .125);
        border-radius: .25rem;
    }

    .card-body {
        flex: 1 1 auto;
        min-height: 1px;
        padding: 1rem;
    }

    .gutters-sm {
        margin-right: -8px;
        margin-left: -8px;
    }

    .gutters-sm>.col,
    .gutters-sm>[class*=col-] {
        padding-right: 8px;
        padding-left: 8px;
    }

    .mb-3,
    .my-3 {
        margin-bottom: 1rem !important;
    }

    .bg-gray-300 {
        background-color: #e2e8f0;
    }

    .h-100 {
        height: 100% !important;
    }

    .shadow-none {
        box-shadow: none !important;
    }
</style>



<x-complainant-layout>


    <div class="container card mt-3">

        <div class=" my-3 mt-4 card border">
            <div class="d-flex justify-content-center">
                <img src="{{ asset('/image/resume.jpg') }}" width="300" alt="">
            </div>
        </div>

        <div class="main-body">

            <div class="row gutters-sm">
                <div class="col-md-4 mb-3">
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex flex-column align-items-center text-center">
                                <img src="https://bootdey.com/img/Content/avatar/avatar7.png" alt="Admin"
                                    class="rounded-circle" width="150">
                                <div class="mt-3">
                                    <h4>
                                        {{ Auth::guard('complainant')->user()->name }}
                                    </h4>
                                    <p class="text-secondary mb-1">
                                        {{ Auth::guard('complainant')->user()->email }}
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-8">
                    <div class="card mb-3">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-sm-3">
                                    <h6 class="mb-0">ชื่อ-นามสกุล :</h6>
                                </div>
                                <div class="col-sm-9 text-secondary">
                                    {{ Auth::guard('complainant')->user()->name }}
                                </div>
                            </div>
                            <hr>
                            <div class="row">
                                <div class="col-sm-3">
                                    <h6 class="mb-0">อีเมล :</h6>
                                </div>
                                <div class="col-sm-9 text-secondary">
                                    {{ Auth::guard('complainant')->user()->email }}
                                </div>
                            </div>
                            <hr>
                            <div class="row">
                                <div class="col-sm-3">
                                    <h6 class="mb-0">โทรศัทพ์ :</h6>
                                </div>
                                <div class="col-sm-9 text-secondary">
                                    {{ Auth::guard('complainant')->user()->phone_num }}
                                </div>
                            </div>
                            <hr>
                            <div class="row">
                                <div class="col-sm-3">
                                    <h6 class="mb-0">เลขบัตรประจำตัวประชาชน :</h6>
                                </div>
                                <div class="col-sm-9 text-secondary">
                                    {{ Auth::guard('complainant')->user()->national_id }}
                                </div>
                            </div>
                            <hr>
                            <div class="row">
                                <div class="col-sm-3">
                                    <h6 class="mb-0">ที่อยู่ :</h6>
                                </div>
                                <div class="col-sm-9 text-secondary">
                                    {{ Auth::guard('complainant')->user()->address }}
                                </div>
                            </div>
                            <hr>
                            <div class="row">
                                <div class="col-sm-12">
                                    <a class="btn btn-outline-info" href="{{ url('profile_edit/' . Auth::guard('complainant')->user()->id) }}">แก้ไขข้อมูล</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>



    {{-- โปรไฟล์
    <a href="{{ url('profile_edit/' . Auth::guard('complainant')->user()->id) }}">แก้ไขข้อมูล</a>
    <br>
    <a href="{{ url('/complainant/dashboard') }}">กลับ</a>
    <br>
    ชื่อ:{{ Auth::guard('complainant')->user()->name }}
    <br>
    อีเมล:{{ Auth::guard('complainant')->user()->email }}
    <br>
    เบอร์โทร:{{ Auth::guard('complainant')->user()->phone_num }}
    <br>
    ที่อยู่:{{ Auth::guard('complainant')->user()->address }}
    <br>
    เลขบัตรประจำตัวประชาชน:{{ Auth::guard('complainant')->user()->national_id }}
    <br> --}}

</x-complainant-layout>
